package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1058:int = 3;
      
      private var var_470:AssetLoaderStruct;
      
      private var var_1057:Timer;
      ...
      private const const_2047:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_470 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_1057 = new Timer(this.const_2047,1);
         this.var_1057.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_470)
         {
            this.var_470.addEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_470.addEventListener(AssetLoaderEvent.const_49,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_470)
         {
            this.var_470.removeEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_470.removeEventListener(AssetLoaderEvent.const_49,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_1058);
            if(this.var_1581.indexOf("?") > 0)
            {
               _loc2_ = this.var_1581 + "&retry=" + this.var_1058;
            }
            else
            {
               _loc2_ = this.var_1581 + "?retry=" + this.var_1058;
            ...
            this.var_470 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2860,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1301[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1301[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_2055)
         {
            this.var_1302.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_493[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_493[_loc3_] = _loc6_;
            }
            this.var_1059[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         for(_loc3_ in this.var_1059)
         {
            ...
            if(_loc7_)
            {
               ...
               delete this.var_493[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_1059[_loc3_];
         }
         while(_loc6_ < this.var_860.length)
         {
            _loc5_ = this.var_860[_loc6_];
            if(_loc5_.method_7 == (param1.target as AvatarAssetDownloadLibrary).method_7)
            {
               this.var_860.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_1057.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_859.length > 0 && this.var_860.length < this.const_1428)
         {
            _loc1_ = this.var_859[0];
            _loc1_.startDownloading();
            this.var_860.push(this.var_859.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_859.indexOf(param1) == -1 && this.var_860.indexOf(param1) == -1)
         {
