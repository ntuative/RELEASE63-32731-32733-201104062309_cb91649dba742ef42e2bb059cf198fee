package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class SpacesNewCatalogWidget extends ItemGridCatalogWidget implements IDisposable, ICatalogWidget, IItemGrid
   {
      ...
      public function SpacesNewCatalogWidget(param1:IWindowContainer)
      {
         ...
         if(this.var_422)
         {
            ...
            while(_loc2_ < this.var_422.numSelectables)
            {
               ...
               {
                  _loc3_.addEventListener(WindowEvent.const_61,this.onSelectGroup);
               }
               _loc2_++;
            }
         }
      ...
      override public function init() : Boolean
      {
         ...
         this.switchCategory(this.var_101[this.var_578]);
      ...
      {
         var _loc2_:int = this.var_707[this.var_578];
         var _loc3_:Offer = this.var_217[this.var_578][_loc2_];
      ...
      override public function select(param1:IGridItem) : void
      {
         ...
         this.var_707[this.var_578] = (this.var_217[this.var_578] as Array).indexOf(_loc2_);
         this.updateRoomPreview();
      }
      ...
      private function createOfferGroups() : Boolean
      {
         ...
         {
            this.var_180 = configurationAsset.content as XML;
            for each(offer in page.offers)
            {
               if(!(offer.pricingModel == Offer.const_365 || offer.pricingModel == Offer.const_372))
               {
                  continue;
               }
               ...
               if(!(product.productType == ProductTypeEnum.const_65 || product.productType == ProductTypeEnum.const_76))
               {
                  continue;
               }
               ...
               {
                  case "floor":
                     for each(pattern in this.var_180.floors.pattern)
                     {
                     ...
                     break;
                  case "wallpaper":
                     pattern = this.var_180.walls.pattern.(@id == offerPostfix)[0] as XML;
                     if(pattern)
                     {
                     ...
                     break;
                  case "landscape":
                     pattern = this.var_180.landscapes.pattern.(@id == offerPostfix)[0] as XML;
                     if(pattern)
                     {
      ...
      private function switchCategory(param1:String) : void
      {
         ...
         if(_loc2_ > -1)
         {
            ...
            page.replaceOffers(this.var_217[this.var_578],false);
            resetTimer();
            populateItemGrid();
            ...
            _loc3_ = this.var_707[this.var_578];
            selectIndex(_loc3_);
         }
      }
   }
