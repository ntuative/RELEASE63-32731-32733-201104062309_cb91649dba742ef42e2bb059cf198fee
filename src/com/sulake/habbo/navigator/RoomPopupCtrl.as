package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_1083 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_188 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_188.visible = true;
         Util.hideChildren(this.var_188);
         this.refreshRoomName(this.var_188,this.var_120);
         if(this.var_120.ownerName != "" && this.var_120.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_188,!!this.var_120.event ? this.getEventCtg(this.var_120.categoryId) : this.getRoomCtg(this.var_120.categoryId));
         this.refreshRoomDesc(this.var_188,this.var_120);
         this.refreshExtraCont();
         this.refreshInfo(this.var_188,"trading_allowed",this.var_120.allowTrading);
         this.refreshInfo(this.var_188,"doormode_doorbell",this.var_120.doorMode == 1);
         this.refreshInfo(this.var_188,"doormode_password",this.var_120.doorMode == 2);
         Util.moveChildrenToColumn(this.var_188,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_188.height = Util.getLowestPoint(this.var_188);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_188,this.var_120.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_188.findChildByName("roomowner"));
         if(this.var_120.ownerName != "" && this.var_120.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_188.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1083.refreshTags(_loc1_,this.var_120.tags);
         ...
         this.var_2610.refreshThumbnail(_loc1_,this.var_120.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_273,Tabs.const_225,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
