package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         this.var_2428 = param1.hasHabboClub;
         this.var_1767 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_1145.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_1145.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_1145.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_1145.toString());
         this.showInterface();
      }
      ...
      private function showInterface() : void
      {
         ...
         {
            switch(this.var_1766)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_409:
                  _loc1_.caption = !!this.var_1767 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1410;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_542:
                  _loc1_.caption = !!this.var_1767 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = this.var_1410;
         }
         var _loc4_:IWindowContainer = this._window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !this.var_2428;
         }
      ...
      {
         messageListener.processWidgetMessage(new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_802));
      }
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_1766 == RoomWidgetRoomQueueUpdateEvent.const_409)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_896);
         }
         messageListener.processWidgetMessage(_loc2_);
         this.removeWindow();
      }
