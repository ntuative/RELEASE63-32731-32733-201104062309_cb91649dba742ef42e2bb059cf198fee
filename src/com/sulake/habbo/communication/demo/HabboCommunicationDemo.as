package com.sulake.habbo.communication.demo
{
   ...
   public class HabboCommunicationDemo extends Component
   {
      ...
      private var var_542:IKeyExchange;
      
      private var var_1008:String;
      
      private var _view:HabboLoginDemoView;
      
      private var var_2722:PseudoRandom;
      ...
      private const const_1734:String = "fuselogin";
      ...
      public function setSSOTicket(param1:String) : void
      {
         ...
         {
            this.var_665 = param1;
            this.var_58.initConnection(HabboConnectionType.const_251);
         }
      }
      ...
      {
         if(this.var_58 && this.var_168 && this.var_15 && this._windowManager)
         {
      ...
      private function componentsAreRunning() : void
      {
         ...
         this.var_231 = new HabboHotelView(this._windowManager,assets,Component(context).events);
         var _loc1_:String = this.var_168.getKey("client.hotel_view.image");
         var _loc2_:String = this.var_168.getKey("image.library.url");
         if(_loc1_ != null && _loc2_ != null)
         {
            this.var_231.loadHotelViewImage(_loc2_ + _loc1_);
         }
         var _loc3_:* = this.var_168.getKey("use.sso.ticket") == "1";
         this.var_665 = this.var_168.getKey("sso.ticket",null);
         this.var_1955 = this.var_168.getKey("flash.client.url","");
         this.var_2724 = this.var_168.getKey("external.variables.txt","");
         if(this.var_231 == null)
         {
            this.var_231 = new HabboHotelView(this._windowManager,assets,Component(context).events);
         }
         if(_loc3_)
         {
            this.var_58.mode = HabboConnectionType.const_93;
            if(this.var_665)
            {
               this.var_58.initConnection(HabboConnectionType.const_251);
            }
         ...
         else if(this._windowManager != null)
         {
            ...
            if(_loc4_)
            {
               ...
               this._view.addEventListener(HabboLoginDemoView.const_586,this.onInitConnection);
            }
         }
      }
      ...
      private function onInitConnection(param1:Event = null) : void
      {
         ...
         {
            this.var_58.mode = HabboConnectionType.const_93;
            if(this.var_168.keyExists("local.environment"))
            {
               this.var_58.mode = HabboConnectionType.const_1093;
            }
         }
         this.var_58.initConnection(HabboConnectionType.const_251);
      }
      ...
      private function onConnectionEstablished(param1:Event = null) : void
      {
         ...
         if(_loc2_ != null)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_885);
      ...
      private function onInitCrypto(param1:IMessageEvent) : void
      {
         ...
         this.var_2722 = new PseudoRandom(parseInt(_loc4_.substring(_loc4_.length - 4),16),65536);
         switch(this.var_58.mode)
         {
            case HabboConnectionType.const_93:
               _loc6_ = "";
               _loc6_ = this.var_168.getKey("url.prefix",_loc6_);
               _loc6_ = _loc6_.replace("http://","");
               _loc6_ = _loc6_.replace("https://","");
               _loc7_ = this.var_168.getKey("hotelview.banner.url","http:/\nsitename$/gamedata/banner");
               _loc7_ = _loc7_.replace("$sitename$",_loc6_);
               this.var_1519 = _loc4_;
               this.var_231.loadBannerImage(_loc7_ + "?token=" + this.var_1519,this.onHotelViewBannerLoaded);
               break;
            case HabboConnectionType.const_1093:
               this.sendConnectionParameters(_loc2_);
               break;
            case HabboConnectionType.const_1469:
               this.var_1008 = this.generateRandomHexString(30);
               this.var_542 = new DiffieHellman(new BigInteger(this.var_168.getKey("connection.development.prime"),16),new BigInteger(this.var_168.getKey("connection.development.generator"),16));
               this.var_542.init(this.var_1008);
               _loc8_ = this.var_542.getPublicKey(10);
      ...
      private function onSecretKeyEvent(param1:IMessageEvent) : void
      {
         ...
         this.var_542.generateSharedKey(_loc4_,10);
         var _loc5_:String = this.var_542.getSharedKey(16).toUpperCase();
         var _loc6_:RC4_R27 = new RC4_R27(null,null);
         var _loc7_:IEncryption = new RC4_R27(_loc6_,this.var_2722);
      ...
      private function sendConnectionParameters(param1:IConnection) : void
      {
         ...
         connection.send(new VersionCheckMessageComposer(401,this.var_1955,this.var_2724));
         var machineId:String = "";
         try
         {
            so = SharedObject.getLocal(this.const_1734,"/");
      ...
      private function onSessionParamsEvent(param1:IMessageEvent) : void
      {
         ...
         if(this.var_665 != null)
         {
            _loc4_ = new SSOTicketMessageComposer(this.var_665);
            _loc2_.send(_loc4_);
      ...
      private function onConnectionDisconnected(param1:Event) : void
      {
         ...
         if(param1.type == Event.CLOSE && !this.var_2723)
         {
            _loc2_ = this.var_168.getKey("logout.disconnect.url");
      ...
      private function onDisconnectReason(param1:DisconnectReasonEvent) : void
      {
         ...
         this.var_2723 = true;
         var _loc2_:String = this.var_168.getKey("logout.url");
         if(_loc2_.length > 0)
         {
      ...
      private function setOriginProperty(param1:String) : String
      {
         ...
         {
            return param1.replace("%origin%",this.var_168.getKey("flash.client.origin","popup"));
         }
         return param1;
      }
      ...
      private function onHotelViewBannerLoaded(param1:BitmapData) : void
      {
         ...
         this.var_542 = new DiffieHellman(_loc10_,_loc11_);
         var _loc13_:int = 10;
         var _loc14_:* = null;
         while(_loc13_ > 0)
         {
            _loc14_ = this.generateRandomHexString(30);
            this.var_542.init(_loc14_);
            _loc15_ = this.var_542.getPublicKey(10);
