package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1361:String = "asset-libraries";
      
      private static const const_1357:String = "library";
      
      private static const const_1358:String = "service-libraries";
      
      private static const const_1355:String = "library";
      
      private static const const_1363:String = "component-libraries";
      
      private static const const_1356:String = "library";
      
      private static const const_1362:String = "error_data";
      
      private static const const_1359:String = "error_cat";
      
      private static const const_1360:String = "error_desc";
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            ...
            case Core.const_861:
               ...
               var_406.addChild(new ProfilerViewer(this.var_207));
               break;
            case Core.const_481:
      ...
      {
         return this.var_541 - this.getNumberOfFilesPending();
      }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1362,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1359,String(param3));
         ErrorReportStorage.setParameter(const_1360,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         try
         {
            node = config.child(const_1361)[0];
            if(node != null)
            {
               list = node.child(const_1357);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_341,e);
         }
         try
         {
            node = config.child(const_1358)[0];
            if(node != null)
            {
               list = node.child(const_1355);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_341,e);
         }
         try
         {
            node = config.child(const_1363)[0];
            if(node != null)
            {
               list = node.child(const_1356);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_341,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1627);
         if(!disposed)
         {
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_804)
                  {
                     this.var_804.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_541 - this.var_382.length,this.var_541,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_207 ? 0 : this.var_1515 & 0;
         if(_loc3_ == Core.const_481)
         {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_103)
         {
            ...
            {
               if(this.var_663[priority] < priority)
               {
                  ++this.var_663[priority];
                  skip = true;
               }
            }
            if(!skip)
